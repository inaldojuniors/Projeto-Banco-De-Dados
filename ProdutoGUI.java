...
public class ProdutoGUI extends Application {
	...
	public void start(Stage palco){
		...
		updateButton.setOnAction(e -> {
			...
			if (selectedProduto != null){
				...
				produtoDAO.atualizar(selectedProduto); // Atualiza produto no banco de dados.
				produtos.setAll(produtoDAO.listarTodos()); // Atualiza a lista de produtos.
				limparCampos(); // Limpa campos de entrada
			}
		});
		...
		deleteButton.setOnAction( e -> {
			...
			if (selectedProduto != null) {
				...
				produtos.setAll(produtoDAO.listarTodos()); // Atualiza lista de produtos
				limparCampos();
			}
		});
	...
	/**
	*O metodo stop é automaticamente chamado quando a aplicação JavaFx é encerrada
	*/
	@Override
	public void stop(){
		try {
	...
	* Para garantir que os campos de entrada estejam prontos para uma nova entrada de dados
	*/
	...
	* @param property A propriedade do objeto que esta coluna deve exibir
